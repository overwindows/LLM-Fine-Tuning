...
def main():
    ...
    model = LlamaForCausalLM.from_pretrained(f"{model_name}", use_cache=False).cuda()
        
    data_name = data_args.data_name_or_path
    dataset=load_dataset("json", data_files=data_name, cache_dir='/import/snvm-sc-podscratch3/chenw/datasets', streaming=True, split='train')
